# Converts PANORAMA dataset labels to include only healthy pancreas and tumor (PDAC in this case)
import os
import numpy as np
...
def remap_single_file(input_filename, input_folder, output_folder):
    ...
    try:
        ...
        # Create a new array initialized with zeros (background label)
        new_data = np.zeros_like(data, dtype=np.int16)
        
        new_data[data == 1] = 1
        new_data[data == 2] = 0 # Pancreas to background
...
if __name__ == "__main__":
    ...
    # Change folder paths here for a PANTHER Task dataset to change any non-tumor labels to background 
    input_directory = "ENTER_YOUR_PATH/nnUNet_raw/Dataset9xx_PANTHER/labelsTr"
    output_directory = "ENTER_YOUR_PATH/nnUNet_raw/Dataset9xx_PANTHER/labelsTr_fixed"
